"""
VGG16 Feature Extractor Training Script

This script trains a VGG16-based model using the pretrained network as a feature
extractor. The convolutional layers are frozen and only the classifier is trained.
...
from ..models.vgg_models import VGG16FeatureExtractor
...
def save_metrics(metrics, output_dir):
    ...
    metrics_file = os.path.join(metrics_dir, 'vgg16_feature_extractor_metrics.txt')
    
    with open(metrics_file, 'w') as f:
        f.write('Model: VGG16 Feature Extractor\n')
...
def plot_training_history(metrics, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(vis_dir, 'vgg16_feature_extractor_history.png'))
    plt.close()
...
def main(args):
    ...
    metrics = trainer.train(train_loader, val_loader, model_save_path, model_name="vgg16_feature_extractor_best.pth")
    ...
    if len(train_loader) > 0:
        ...
        ax.axis('off')
        ax.set_title('VGG16 Feature Maps')
        plt.savefig(os.path.join(vis_dir, 'vgg16_feature_extractor_maps.png'))
        plt.close()


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train VGG16 Feature Extractor')
