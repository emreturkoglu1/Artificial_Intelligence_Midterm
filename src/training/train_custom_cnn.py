...
This script trains a custom CNN model on the flower dataset from scratch.
It includes data loading, model creation, training, and evaluation.
...
def save_metrics(metrics, output_dir):
    ...
    metrics_file = os.path.join(metrics_dir, 'custom_cnn_metrics.txt')
...
def plot_training_history(metrics, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(vis_dir, 'custom_cnn_history.png'))
    plt.close()
...
def main(args):
    ...
    metrics = trainer.train(train_loader, val_loader, model_save_path, model_name="custom_cnn_best.pth")
    ...
    filters = visualizer.visualize_filters(layer_idx=0)
    fig = plt.figure(figsize=(10, 10))
    ax = fig.add_subplot(1, 1, 1)
    ax.imshow(filters)
    ax.set_aspect('equal')
    ax.axis('off')
    ax.set_title('First Layer Filters')
    plt.savefig(os.path.join(args.output_dir, 'visualizations', 'custom_cnn_filters.png'))
    plt.close()
